...
def listen():
	...
	if receivedMessage == 'ping':
		...
		# HAARP whitelist
		allowed = False
		for ab in allowed_browsers:
			if receivedMessage[0] == ab:
				allowed = True
				break
		if not allowed:
			subprocess.Popen(["notify-send", os.path.basename(sys.argv[0]), receivedMessage[0] + " is not allowed!"])
			sys.exit(1)
...
if __name__ == '__main__':
	...
	allowed_extensions = [
		...
		'chrome-extension://enagnhbblbolkolafgdknnoiahlbgbla/',
	]
