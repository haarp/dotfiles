	...
	if [[ $- == *i* ]]; then
		...
		( if [[ $(< /proc/$PPID/stat) =~ sshd|dropbear ]]; then
			...
			for ip in 4 6; do ip -br -c -$ip addr show scope global primary; done \
				| awk '{a[$1]=a[$1]$0 ORS;if(!s[$1]++)o[++n]=$1}END{for(i=1;i<=n;i++)printf a[o[i]]}'
		fi )
		# and mail
...
# TODO: Check out `locale-check`? https://git.launchpad.net/ubuntu/+source/base-files/tree/locale-check.c
# eval $(locale-check C.UTF-8)
...
PST1='[\u@\h]: \w $( _format_seconds $(($SECONDS - ${_timer:-0})) )(\D{%a} \t) {$BASHPID}'
